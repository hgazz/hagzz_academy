...
@php
    ...
    $copy = $ar ? [
        ...
        'all' => 'الكل', 'subscriptions' => 'حسابات واشتراكات الطلاب', 'training' => 'حجوزات التدريبات', 'venues' => 'حجوزات الملاعب', 'camps' => 'معسكرات تدريبية',
        ...
        <main class="financial-reports" dir="{{ $ar ? 'rtl' : 'ltr' }}">
            ...
            <section class="fr-breakdown">
                @foreach($breakdown as $source => $totals)
                    <a class="fr-source-card" href="#{{ $source }}-report">
                        <div class="fr-source-head">
                            <i class="fa-solid {{ $source === 'subscriptions' ? 'fa-user-graduate' : ($source === 'training' ? 'fa-ticket' : ($source === 'venues' ? 'fa-futbol' : 'fa-campground')) }}"></i>
                            <strong>{{ $sourceLabels[$source] ?? $source }}</strong>
                        </div>
                        <div class="fr-source-values">
                            <span>{{ $copy['collected'] }} <b>{{ $money($totals['collected']) }}</b></span>
                            <span>{{ $copy['remaining'] }} <b>{{ $money($totals['remaining']) }}</b></span>
                        </div>
            ...
            <section class="fr-filter-panel">
                <div class="fr-section-heading">
                    <div><span>{{ $copy['filters'] }}</span><h2>{{ $copy['details'] }}</h2></div>
                ...
                <form method="GET" action="{{ route('academy.report.overview') }}" class="fr-filter-form">
                    ...
                    <div class="fr-filter-actions">
                        <button type="submit"><i class="fa-solid fa-filter"></i>{{ $copy['apply'] }}</button>
                ...
                <section class="fr-report-panel" id="subscriptions-report">
                    <header>
                        <div>
                            ...
                            <div><h2>{{ $copy['subscriptions'] }}</h2><p>{{ $copy['details'] }}</p></div>
                        </div>
                        <a href="{{ route('academy.report.overview.export', array_merge(['type' => 'subscriptions'], $queryFilters)) }}">
                            <i class="fa-solid fa-download"></i>{{ $copy['export'] }}
                        </a>
                    </header>
                        ...
                        <table>
                            ...
                            <tbody>
                                @forelse($subscriptions as $subscription)
                                    @php
                                        $paid = (float) ($subscription->payments_sum_amount ?? 0);
                                        $remaining = max(0, (float) $subscription->amount - $paid);
                                        $pStatus = $subscription->payment_status;
                                        $pStatusLabel = $paymentLabels[$pStatus] ?? $pStatus;
                                    @endphp
                                    <tr>
                                        <td>{{ $subscription->id }}</td>
                                        <td><strong>{{ $subscription->student?->name ?: '-' }}</strong><small>{{ $subscription->student?->phone ?: '-' }}</small></td>
                                        <td>{{ $subscription->group?->name ?: '-' }}</td>
                                        <td><strong>{{ $subscription->starts_on?->format('Y-m-d') }}</strong><small>{{ $subscription->ends_on?->format('Y-m-d') }}</small></td>
                                        <td>{{ $money($subscription->amount) }}</td>
                                        <td class="is-positive">{{ $money($paid) }}</td>
                                        <td class="{{ $remaining > 0 ? 'is-negative' : '' }}">{{ $money($remaining) }}</td>
                                        <td>
                                            <span class="fr-status is-{{ $pStatus }}">{{ $pStatusLabel }}</span>
                                            <small>{{ $subscription->status }}</small>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="8" class="fr-empty">{{ $copy['noData'] }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <section class="fr-report-panel" id="training-report">
                    <header>
                        <div>
                            ...
                            <div><h2>{{ $copy['training'] }}</h2><p>{{ $copy['details'] }}</p></div>
                        </div>
                        <a href="{{ route('academy.report.overview.export', array_merge(['type' => 'training'], $queryFilters)) }}">
                            <i class="fa-solid fa-download"></i>{{ $copy['export'] }}
                        </a>
                    </header>
                    <div class="fr-table-wrap">
                        <table>
                            ...
                            <tbody>
                                @forelse($trainingBookings as $booking)
                                    @php
                                        $pState = $booking->payment_state;
                                        $pStateLabel = $paymentLabels[$pState] ?? $pState;
                                    @endphp
                                    <tr>
                                        <td><strong>{{ $booking->order_number ?: '#' . $booking->id }}</strong><small>{{ $booking->user_type }}</small></td>
                                        <td><strong>{{ $booking->user?->name ?: '-' }}</strong><small>{{ $booking->user?->phone ?: '-' }}</small></td>
                                        <td>{{ $booking->training?->name ?: '-' }}</td>
                                        <td>{{ $booking->created_at?->format('Y-m-d') }}</td>
                                        <td>{{ $money($booking->amount) }}</td>
                                        <td class="is-positive">{{ $money($booking->collected_amount) }}</td>
                                        <td class="{{ $booking->remaining_amount > 0 ? 'is-negative' : '' }}">{{ $money($booking->remaining_amount) }}</td>
                                        <td>{{ $booking->payment_method_label }}</td>
                                        <td>
                                            @if($booking->is_canceled)
                                                <span class="fr-status is-cancelled">{{ $ar ? 'ملغي' : 'Cancelled' }}</span>
                                            @else
                                                <span class="fr-status is-{{ $pState }}">{{ $pStateLabel }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="9" class="fr-empty">{{ $copy['noData'] }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    {{ $trainingBookings->links() }}
                </section>
            @endif
                ...
                <section class="fr-report-panel" id="venues-report">
                    <header>
                        <div>
                            ...
                            <div><h2>{{ $copy['venues'] }}</h2><p>{{ $copy['details'] }}</p></div>
                        </div>
                        <a href="{{ route('academy.report.overview.export', array_merge(['type' => 'venues'], $queryFilters)) }}">
                            <i class="fa-solid fa-download"></i>{{ $copy['export'] }}
                        </a>
                    </header>
                    <div class="fr-table-wrap">
                        <table>
                            ...
                            <tbody>
                                @forelse($venueBookings as $booking)
                                    @php
                                        $vStatus = $booking->payment_status;
                                        $vStatusLabel = $paymentLabels[$vStatus] ?? $vStatus;
                                    @endphp
                                    <tr>
                                        <td><strong>{{ $booking->reference }}</strong><small>{{ $booking->source }}</small></td>
                                        <td><strong>{{ $booking->customer?->name ?: '-' }}</strong><small>{{ $booking->customer?->phone ?: '-' }}</small></td>
                                        <td><strong>{{ $booking->space?->venue?->name ?: '-' }}</strong><small>{{ $booking->space?->name ?: '-' }}</small></td>
                                        <td><strong>{{ $booking->starts_at?->format('Y-m-d') }}</strong><small>{{ $booking->starts_at?->format('H:i') }} – {{ $booking->ends_at?->format('H:i') }}</small></td>
                                        <td>{{ $money($booking->total_amount) }}</td>
                                        <td class="is-positive">{{ $money($booking->paid_amount) }}</td>
                                        <td class="{{ $booking->remaining_amount > 0 ? 'is-negative' : '' }}">{{ $money($booking->remaining_amount) }}</td>
                                        ...
                                        <td>
                                            @if($booking->status === 'cancelled')
                                                <span class="fr-status is-cancelled">{{ $ar ? 'ملغي' : 'Cancelled' }}</span>
                                            @else
                                                <span class="fr-status is-{{ $vStatus }}">{{ $vStatusLabel }}</span>
                ...
                <section class="fr-report-panel" id="camps-report">
                    <header>
                        <div>
                            ...
                            <div><h2>{{ $copy['camps'] }}</h2><p>{{ $copy['details'] }}</p></div>
                        </div>
                        <a href="{{ route('academy.report.overview.export', array_merge(['type' => 'camps'], $queryFilters)) }}">
                            <i class="fa-solid fa-download"></i>{{ $copy['export'] }}
                        </a>
                    </header>
                    <div class="fr-table-wrap">
                            ...
                            <tbody>
                                @forelse($campParticipants as $p)
                                    ...
                                    <tr>
                                        ...
                                        <td><strong>{{ $p->name }}</strong><small>{{ $p->phone }}</small></td>
                                        <td><strong>{{ $p->camp?->title_ar ?: 'معسكر تدريبي' }}</strong></td>
                                        <td>{{ $p->created_at?->format('Y-m-d') }}</td>
                                        <td>{{ $money($p->total_fee) }}</td>
                                        <td class="is-positive">{{ $money($p->paid_amount) }}</td>
                                        <td class="{{ $rem > 0 ? 'is-negative' : '' }}">{{ $money($rem) }}</td>
                                        <td><span class="fr-status is-{{ $p->status === 'confirmed' ? 'paid' : 'partial' }}">{{ $p->status }}</span></td>
                                    </tr>
                                @empty
                                    <tr><td colspan="8" class="fr-empty">{{ $copy['noData'] }}</td></tr>
                                @endforelse
